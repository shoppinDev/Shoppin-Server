	...
	switch($webMethod)
	{
		case "merchant_register":
			...
			if($num_rowsc>0) 
			{
				...
				//echo date("Y-m-d"); die;
				$query = "INSERT INTO `deal_merchant` (`merchant_email`, `merchant_password`, `added_date`) VALUES ('".$email."', '".$password."','".date("Y-m-d")."')";
				//echo $query; die;
		...
		case "adddeal":
			...
			$query = "INSERT INTO `deal_deals` (`merchant_id`, `shop_id`, `deal_category`, `deal_subcategory`, `deal_title`, `deal_description`, `deal_startdate`, `deal_enddate`, `deal_amount`, `all_days`, `deal_usage`, `location`, `discount_type`, `discount_value`,`added_date`) VALUES ('".$merchant_id."', '".$shop_id."' , '".$deal_category."' , '".$deal_subcategory."' , '".$deal_title."' , '".$deal_description."' , '".$deal_startdate."' , '".$deal_enddate."' ,  '".$deal_amount."' , '".$all_days."' , '".$deal_usage."' , '".$location."'  , '".$discount_type."'  , '".$discount_value."' , '".date("Y-m-d")."')";
		...
		case "my_shop":
			...
			$query = " SELECT d.*,  dm.merchant_name from deal_merchantshops d  
						left join deal_merchant dm on dm.merchant_id = d.merchant_id 
						where d.shop_id <> 0 and is_active = 1 and d.merchant_id = '".$merchantid."' "; 
			packupAndSend($query,$link);		
		break;
