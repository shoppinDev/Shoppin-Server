  ...
  <section id="content_wrapper">
    <div id="topbar">
      <div class="topbar-left">
        <ol class="breadcrumb">
		  <li class="crumb-active"><a href="#"> Edit Admin User <?php echo $admin_name;?></a></li>
          ...
          <li class="crumb-trail">Edit Admin User</li>
        </ol>
      </div>
    </div>
    <div id="content">
      <div class="row">
        <div class="col-md-12">
          <div class="panel">
            <div class="panel-heading"> <span class="panel-title"> <span class="glyphicon glyphicon-lock"></span> Edit Admin User <?php echo $admin_name;?> </span> </div>
...
<?php } ?>
  ...
  <div class="resp-tabs-container">
    ...
    <div class="static-page-data">
     ...
     <form role="form" id="form" method="post" action="<?php echo $base_url;?>admin_user/edit/<?php echo $admin_id; ?>" enctype="multipart/form-data" >
 				
			<INPUT TYPE="hidden" NAME="hidPgRefRan" VALUE="<?php echo rand();?>">
			<INPUT TYPE="hidden" NAME="action" VALUE="edit_admin_user">
			<INPUT TYPE="hidden" NAME="hiddenaction" VALUE="<?php echo $admin_id;?>">
			<?php if($approval!=''){ ?>
	...
	<!--		<div class="form-group">
				...
				-->
                  ...
                  <label for="admin_role">Admin Role<span style="color:red"> *<span></label>
				  	<span id='prod1'> 
				<select name='admin_role_id' id="admin_role_id" class="form-control jobtext">
				<option value="" selected>-- Select Admin Role--</option>
				<?php for($i=0;$i<count($allrole);$i++)
				{
				?>
				<option value='<?php echo $allrole[$i]->role_id; ?>'<?php if($allrole[$i]->role_id==$admin_role_id){ echo 'selected'; } ?>>
			...
			<!--	<div class="form-group">
                    ...
                    <div class="radio">
                        <label>
                        <input type="radio" id="status" name="status" value="1" <?php if($status=='1'){echo 'checked';}?>> 
                         Active
                        </label>
                      </div>
                      <div class="radio">
                        <label>
                          <input type="radio" id="status" name="status"  value="0" <?php if($status=='0'){echo 'checked';}?>>
                          Deactive
                        </label>
                      </div>                     
					</div>
				...
				-->
					...
					<div class="form-group">
					 	<?php if($status=='1')
										{ 
										?>
										 <input class="submit btn bg-purple pull-right" id="is_active" type="button" id="is_active" name="is_active"  value="Inactive" onclick="deapprove('<?php echo $base_url."admin_user/userstatus/";?><?php echo $admin_id; ?>',0);"/>
										
				  
										<?php }else{ ?>
										 <input class="submit btn bg-purple pull-right" id="is_active" name="is_active" type="button" value="Active" onclick="approve('<?php echo $base_url."admin_user/userstatus/";?><?php echo $admin_id; ?>',1);"/>
...
<script>
	...
	var admin_role_id= $("#admin_role_id").val();
		if(admin_role_id==''){
			
			$("#error_msg1").html("Please Select Admin User Type.");
		...
		if(admin_name==''){
			...
			$("#error_msg1").html("Please Enter The Admin User Name.");
			$("#validator").css("display","block");
			return false;
		}	
...
function approve(url,is_approve){
	//alert(is_approve);
		...
		//alert(is_approve);
		if(is_approve=='1'){
		 alert('Are you sure you want to Active Register Admin User ?');
			window.location.href='<?php echo $base_url ?>/admin_user/userstatus/<?php echo $id ?>/'+is_approve;		 
			}
	...
	//alert(url);
		...
		//alert(is_approve);
		if(is_approve=='0'){
		alert('Are you sure you want to Inactive Register Admin User ?');
			window.location.href='<?php echo $base_url ?>/admin_user/userstatus/<?php echo $id ?>/'+is_approve;	
				} 
