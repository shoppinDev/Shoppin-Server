...
class Merchant_model extends CI_Model {
	...
	function add($content) 
	{
		...
		$data['merchant_password'] = $content['password'];
		...
		$data['merchant_pan'] = $content['pan'];
				$data['merchant_add'] = $content['merchant_add'];
				$data['merchant_city'] = $content['merchant_city'];
				$data['merchant_State'] = $content['merchant_State'];
				$data['merchant_zip'] = $content['merchant_zip'];
				$data['merchant_country'] = $content['merchant_country'];
		$data['is_approve'] = '2' ;			
	...
	function edit($id, $content) {
		...
		$data['merchant_password'] = $content['merchant_password'];
	...
	function list_user($num, $offset, $content) {
		...
		$sql = "SELECT * FROM deal_merchant where merchant_id <> 0 ";
		if($content['name'] != '') 
		{
		...
		if($num!='' || $offset!='')
		{
			$sql .=	" order by merchant_id desc limit ".$offset.",".$num."";
		}
		else
		{
		...
		$sql_count = "SELECT * FROM deal_merchant where merchant_id <> 0  ";
		if($content['name'] != '') 
		{
...
function get_deal_count($id){
		$sql   = "select * from deal_deals where merchant_id='".$id."'";
		$query = $this->db->query($sql);
		$ret = $query->num_rows();
	   	 return $ret;
			}		
